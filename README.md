...
Most of the functionality described in the original project plan was implemented (list filtering missing). More resources were used than initally resourced. This was at least partially caused by the long span on which the project was implemented (after failing the initial schedule). Some work was required updating platform & development tools and moving away from deprecated APIs during the project. Time was also used with the UCrop library, trying to modify the library to the new permission API with bad success. Trying this was however a deliberate decision and risks were weighed against the extended schedule.

The original schedule was somewhat utopistic as I was working more or less fulltime, but after furthering development I feel that the project goals were accomplished satisfactorily. Some deviation occured from the initial UI and architechture plans, but to my experience this is quite common in modern agile software development. Overall, I consider the project successful, despite failing to keep up with the initial schedule.
